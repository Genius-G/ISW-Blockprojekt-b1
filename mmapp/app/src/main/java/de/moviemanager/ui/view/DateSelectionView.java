...
public class DateSelectionView extends FrameLayout {
    ...
    public void setSelectDateEnabled(boolean enabled) {
        selectDate.setEnabled(enabled);
        if(enabled) {
            selectDate.setColorFilter(getContext().getColor(R.color.dark_red));
        } else {
            selectDate.setColorFilter(getContext().getColor(R.color.light_gray));
        }
    }
